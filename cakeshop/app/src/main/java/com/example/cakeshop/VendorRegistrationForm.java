...
public class VendorRegistrationForm extends AppCompatActivity{
    ...
    EditText editUser, editPass, editRepass,editEmail,editAddress,editCon,editFname,editLname,editShop;
    TextView login;
    Button bntSignup;
    ...
    private void initWidgets(){
        ...
        bntSignup = (Button)findViewById(R.id.btnOnSignupx);
        login=(TextView)findViewById(R.id.loginx);
    ...
    private  void toLogin(){
        ...
        Intent login = new Intent(VendorRegistrationForm.this,MainActivity.class);
        startActivity(login);
    }
    ...
    private  void onSignup(View v){
         ...
         *
         * */
        EditText[] ifVendor = {this.editUser,this.editPass,this.editRepass,this.editFname,this.editLname,this.editEmail,this.editCon,this.editAddress,this.editShop};
        if(!this.isValidForm() || this.isFormHasEmpty( ifVendor)){
            ...
            ifVendor = this.remoteAt(ifVendor,2);
            boolean isSuccessfulLogin = this.dbutilities.registerAccount(ifVendor);
    ...
    private void addListeners(){
        ...
        if(!TemporaryState.isCustomer){
            this.editShop.addTextChangedListener(new TextWatcher() {
                @Override
                public void beforeTextChanged(CharSequence s, int start, int count, int after) {}
                @Override
                public void onTextChanged(CharSequence s, int start, int before, int count) {}
                @Override
                public void afterTextChanged(Editable s) {
                    // check if shopname is not epmty
                    if(!s.toString().isEmpty()){
                        formGroup[8] = true;
                    }else{
                        // please modify error message
                        editShop.setError("Please enter shopname");
                        formGroup[8] = false;
                    }
                }
            });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.vendor_registration_form);
        this.initWidgets();
        this.addListeners();
    }
    @Override
